"""Mini interfaz web y worker para configurar alertas 4h."""
...
from monitoring.telegram_alerts import evaluate_candles, fetch_confirmed_candles, format_alert, send_telegram
from scripts.alert_4h import next_4h_close
import time
...
<title>Alertas Trading 4H</title><style>
...
</style></head><body><main><h1>Alertas de velas 4H</h1><p class="muted">La comprobación se realiza una vez después de cada cierre UTC: 00, 04, 08, 12, 16 y 20.</p>
<form id="form"><div class="row"><div><label>Símbolo</label><input name="symbol" value="BTC-USDT-SWAP" required></div><div><label>Precio</label><input name="price" type="number" step="any" required></div></div>
<div class="row"><div><label>Condición</label><select name="direction"><option value="above">Cerrar por encima</option><option value="below">Cerrar por debajo</option></select></div><div><label>Patrón</label><select name="require_engulfing"><option value="true">Exigir envolvente</option><option value="false">Solo nivel de precio</option></select></div></div>
...
async function load(){const r=await fetch('/api/alerts');const a=await r.json();list.innerHTML=a.length?'<h2>Alertas configuradas</h2>'+a.map((x,i)=>`<div class="alert"><div><b>${x.symbol}</b><br>${x.direction==='above'?'Por encima':'Por debajo'} de <b>${x.price}</b><br><span class="muted">${x.require_engulfing?'Con envolvente':'Solo precio'} · ${x.enabled?'Activa':'Pausada'}</span></div><button class="danger" onclick="removeAlert(${i})">Eliminar</button></div>`).join(''):'<p class="muted">No hay alertas configuradas.</p>'}
form.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(form));d.price=Number(d.price);d.require_engulfing=d.require_engulfing==='true';d.enabled=true;await fetch('/api/alerts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)});form.reset();load()};
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if urlparse(self.path).path == "/api/alerts": return self._json(load_config())
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if data.get("direction") not in ("above", "below") or not data.get("symbol") or float(data["price"]) <= 0:
                raise ValueError
...
def worker() -> None:
    while True:
        target = next_4h_close()
        time.sleep(max(0, (target.timestamp() + CHECK_DELAY) - time.time()))
        for item in load_config():
            if not item.get("enabled", True): continue
            try:
                candles = fetch_confirmed_candles(item["symbol"], "4h", 5)
                alert = evaluate_candles(item["symbol"], candles, level=item.get("price"), level_direction=item.get("direction"), require_engulfing=item.get("require_engulfing", True))
                if alert:
