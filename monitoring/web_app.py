...
<form id="form"><div class="row"><div><label>Símbolo</label><input name="symbol" value="BTC-USDT-SWAP" required></div><div><label>Modo</label><select name="mode"><option value="close">Al cierre de vela</option><option value="live">En vivo al tocar el precio</option></select></div></div><div class="row"><div><label>Precio objetivo</label><input name="price" type="number" step="any" required></div><div class="close-only"><label>Timeframe</label><select name="timeframe"><option value="4h">4 horas</option><option value="3h">3 horas</option></select></div></div><div class="row close-only"><div><label>Condición</label><select name="direction"><option value="above">Por encima</option><option value="below">Por debajo</option></select></div><div><label>Patrón</label><select name="require_engulfing"><option value="true">Exigir envolvente</option><option value="false">Solo nivel de precio</option></select></div></div><p id="live-help" class="muted" hidden>Te avisará cuando el precio cruce este nivel, ya sea subiendo o bajando.</p>
...
form.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(form));d.price=Number(d.price);d.require_engulfing=d.require_engulfing==='true';d.enabled=true;if(d.mode==='live')delete d.direction;await fetch('/api/alerts',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(d)});form.reset();syncMode();updatePrice();load()};
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            mode = data.get("mode")
            if mode not in ("live", "close") or data.get("timeframe") not in ("3h", "4h") or not data.get("symbol") or float(data["price"]) <= 0:
                raise ValueError
